...
describe Course do 
	...
	it "Does not allow a course info with an empty string" do
		...
		course = Course.new
		course.createCourses("name", "", "department", 123).should eq(-2)
	end
	...
	it "Does not allow a course info that is not a string" do
		...
		course = Course.new
		course.createCourses("name",007, "department", 123).should eq(-2)
	end
	...
	it "Does not allow a department that is not a string" do
		...
		course = Course.new
		course.createCourses("name","course_info", 007, 123).should eq(-3)
	end
	...
	it "Does allow a course with the right parameters get added" do
		...
		course = Course.new
		course.createCourses("name", "course_info", "department", "007").should eq(1)
	end

	it "Does not allow a course already in the DB to get added" do
		puts "\nTest 9: Course already in DB doesn't get added"
		course = Course.new
		course.createCourses("name", "course_info", "department", "007")
		course.createCourses("name", "course_info", "department", "007").should eq(-5)
	end
